...
- "tool"         → for in-process tools: add_task, add_note, list_tasks, list_notes, search_memory, list_skills, AND db_list_projects/db_describe_schema/db_query_table for the user's CONNECTED Supabase projects (Shoutout, etc. — anything they added in Settings → Data Sources). NEVER pick "tool" for Gmail, Calendar, Drive, Slack, Notion, Linear, or any external service that's not wired as an MCP tool — those go through "skill".
- "skill"        → user is asking you to *do* something that touches an external service (Gmail/Calendar/Drive/Slack/etc.), OR a multi-step action, scheduling, recurring automation, or any operation needing OAuth/API credentials. ALSO pick this for live data queries against JAI's own tables ("what are my tasks", "what skills have I run") when the answer needs synthesis or formatting that's heavier than a plain table lookup. For connected EXTERNAL Supabase projects (anything the user added as a Data Source — Shoutout, etc.), prefer "tool" — those run in-process via PostgREST and are ~15x faster than the skill path.
